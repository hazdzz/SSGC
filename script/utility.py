...
def calc_ssgc_feature(gso, feature, K, beta, device):
    if device == torch.device('cpu'):
        feature_term = feature
        feature = beta * feature
        for k in range(K):
            # (k+1)-th term
            feature_term = gso.dot(feature_term)
            feature = feature + (1 - beta) / K * feature_term
    else:
        feature_term = feature
        feature = beta * feature
        if gso.is_sparse:
            for k in range(K):
                # (k+1)-th term
                feature_term = torch.sparse.mm(gso, feature_term)
                feature = torch.add(feature, (1 - beta) / K * feature_term)
        else:
            for k in range(K):
                # (k+1)-th term
                feature_term = torch.sparse.mm(gso, feature_term)
